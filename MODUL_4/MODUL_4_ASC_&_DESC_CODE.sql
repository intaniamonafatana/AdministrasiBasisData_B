...
FROM tr_penjualan GROUP BY nama_produk ORDER BY total_penjualan DESC;



-- tampilkan total qty yang terjual setiap bulan  yang bener
SELECT *FROM tr_penjualan;
SELECT SUM(qty), tgl_transaksi FROM tr_penjualan AS total_terjual 
GROUP BY month(tgl_transaksi) HAVING SUM(tgl_transaksi);





SELECT qty, MONTH(tgl_transaksi) AS bulan, SUM(qty) AS total_terjual
FROM tr_penjualan GROUP BY MONTH(tgl_transaksi)
ORDER BY bulan;

SELECT qty, YEAR(tgl_transaksi) AS tahun,MONTH(tgl_transaksi) AS bulan,
SUM(qty) AS total_terjual FROM tr_penjualan
...
SELECT qty, SUM(qty) FROM tr_penjualan 
GROUP BY nama_produk HAVING SUM(qty) ORDER BY SUM(qty);
...
SELECT tgl_transaksi, qty, SUM(qty) FROM tr_penjualan AS total_terjual where qty AND month(tgl_transaksi) GROUP BY tgl_transaksi;
SELECT tgl_transaksi, nama_produk, SUM(qty) AS total_terjual FROM tr_penjualan GROUP BY tgl_transaksi;
